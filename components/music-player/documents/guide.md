...
```html
<script src="/你的路徑/components/music-player/js/music-player-component.js"></script>
```
...
```html
<music-player
  data-endpoint="/你的路徑/getMusicData"
  default-volume="0.7"
  default-repeat="true"
...
設定 `data-url` 後，播放器會在初始化時自動載入 JSON 並顯示音樂清單，不需額外撰寫 `fetch + setMusicData()`。
...
- **`data-url`**：靜態 JSON 路徑，組件會自動載入該檔案並初始化音樂清單。

  **資料來源優先順序**：
  1. HTML 屬性 `data-endpoint`（最高優先）
  2. HTML 屬性 `data-url`
  3. 全域變數 `window.MUSIC_PLAYER_ENDPOINT`
  4. 全域變數 `window.MUSIC_PLAYER_DATA_URL`

  如果都不設定，組件不會自動載入資料，需手動呼叫 `setMusicData()`。

- **`resource-path`**：設定資源路徑前綴（預設值請以實際部署環境為準，通常不需填寫）。
...
1. 後端建立 API endpoint，從資料庫取得音樂資料
...
- 組件會自動透過 endpoint 載入資料（優先順序：`data-endpoint` > `data-url` > 全域變數）
- API 需返回 `state: true` 且 `data` 為音樂陣列才會初始化播放器
- 如果都不設定資料來源，需手動呼叫 `setMusicData()` 載入資料
...
2. 在 `<music-player>` 上設定 `data-url` 指向 JSON
3. 組件初始化時自動載入並套用資料
...
- **相對路徑**：其他路徑會自動加上 `resource-path` 屬性設定的前綴（預設值依實際部署環境而定）
...
- 其他格式 → 相對路徑，會加上 `resource-path`，例如：`audio/song.mp3` → `/你的路徑/components/music-player/audio/song.mp3`
...
建立 `music-data.json`（建議放在 `/你的路徑/data/` 目錄下）：
...
<html>
  ...
  <body>
    <!-- Web Component -->
    <music-player default-volume="0.7" default-repeat="false"></music-player>
    ...
    <script>
      ...
      customElements.whenDefined("music-player").then(() => {
        // 讀取 JSON 檔案
        fetch("/你的路徑/data/music-data.json")
...
若由後端提供資料，只需讓 endpoint 回傳以下格式即可（後端語言不限）：
...
```json
{
  ...
  "data": [
    {
      "id": "music1",
      "src": "/你的路徑/media/music/song1.mp3",
      "title": "歌曲名稱 1",
      "image": "/你的路徑/images/music/cover1.jpg",
      "lengthSeconds": 180,
      "isDefault": true
    }
  ]
